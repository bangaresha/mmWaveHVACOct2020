...
rABC = 1*lambda; % Radius of the outer circular ABC boundary in wavelengths
h = 0.04*lambda; % Discretization size in wavelengths
len = 0.5*lambda; % Length of the waveguide
lenA = 1*lambda; % Length of the waveguide

numberOfPDE = 1;
% modelW = createpde;
% gmW = multicylinder([rCyl rCyl+0.1],len,'Void',[true,false]);
% gmA = multicylinder([rCyl+0.1 rABC],lenA);
% % gm = gmA - gmW;
% %gm = multicylinder(rCyl,len);
% modelW.Geometry = gmW;
% figure
% pdegplot(modelW,'CellLabels','on','FaceAlpha',0.5);
...
% modelA.Geometry = gmA;
% figure
% pdegplot(modelA,'CellLabels','on','FaceAlpha',0.5);
% title 'Geometry With Edge Labels Displayed';
% xlabel x
% ylabel y
% zlabel z
...
% mesW = generateMesh(modelW,'GeometricOrder','linear');
% figure
% pdemesh(modelW);
% xlabel x
% ylabel y
% 
% mesA = generateMesh(modelA,'GeometricOrder','linear');
% figure
% pdemesh(modelA);
% xlabel x
% ylabel y

mesAnodesZ = mesA.Nodes(3,:);
mesWnodesZ = mesW.Nodes(3,:);
mesAnodesX = mesA.Nodes(1,:);
mesAnodesY = mesA.Nodes(2,:);
mesWnodesX = mesW.Nodes(1,:);
mesWnodesY = mesW.Nodes(2,:);
% nToDel = [];
% for zzA = 1:length(mesAnodesZ)
%     for zzW = 1:length(mesWnodesZ)
%         if mesAnodesZ(zzA) >= 0.1+mesWnodesZ(zzW) && mesWnodesZ(zzW) <= mesAnodesZ(zzA)-0.1
%             rA = sqrt((mesAnodesX(zzA))^2 + (mesAnodesY(zzA))^2);
%             rW = sqrt((mesWnodesX(zzW))^2 + (mesWnodesY(zzW))^2);
%             if rA >= 0.1+rW && rW <= rA-0.1
%                 nToDel = [nToDel mesAnodesZ(zzW)];
%             end
%         end
%     end
% end

for i = 1:length(nToDel)
    for j = 1:length(mesAnodesZ)
        if mesAnodesZ(j) == nToDel(i)
            mesAnodesZ(j) = 0;
            mesAnodesX(j) = 0;
            mesAnodesY(j) = 0;
        end
...
Ez = Kglobal\btilde;
figure
pdeplot3D(modelW,'ColorMapData',abs(Ez))
